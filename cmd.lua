...
template.Text = "error occured, dm ark"
...
local function getGameList()
	if not isfile("allos games.json") then
		writefile("allos games.json", "[]") 
		return {} 
	end

	local content = readfile("allos games.json")
...
local function logGameLocally()
	...
	writefile("allos games.json", game:GetService("HttpService"):JSONEncode(games))
end
...
local scrollText = "ALLOSTRA made by ark | type cmds for a list of commands | "
...
debugPrint("ALLOSTRA loaded successfully!")