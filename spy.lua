...
do -- Set properties
	...
	objects["Instance0"]["Name"] = "ALLOSTRA";
	...
	objects["Instance4"]["Text"] = "ALLOSTRA~Spy | v1.0.0";
	...
	objects["Instance63"]["Text"] = "PC exploits have become extremely unstable these days; ALLOSTRA~Spy is a very powerful tool that logs EVERYTHING that happens - meaning that your executor may crash when you click \"Load\". You can click these arrows to switch modes, so script would work a little bit more stable (depends).\n\nI recommend that you do not use your main account for exploiting, as you may get banned. Use this script at your own risk.";
	...
	task.spawn(function() -- Instance1
		...
		if not game:GetService("UserInputService").TouchEnabled and game:GetService("UserInputService").KeyboardEnabled and (getfenv().hookmetamethod and getfenv().getnamecallmethod or getfenv().getcallbackvalue or getfenv().hookfunction) then
			...
			local modes = {
				...
				},
				{
					...
					function()
						notification("Will unload ALLOSTRA and load SimpleSpy", "Mode")
					end
				}
			}
		...
		if getfenv().hookmetamethod and getfenv().getnamecallmethod and hf then
			if getfenv().getnilinstances and getfenv().getinstances and getfenv().firesignal and getfenv().getcallbackvalue then
				task.spawn(notification, "That executor fully supports ALLOSTRA~Spy (and simple spy)", "Supported")
			else
				task.spawn(notification, "That executor not fully supports ALLOSTRA~Spy (but possibly supports simple spy)", "Supported")
			end
		else
			task.spawn(notification, "That does not fully support ALLOSTRA~Spy (simple spy probably wont work)", "Unsupported")
		end
		...
		task.spawn(function()
			...
			notification("ALLOSTRA~Spy fully loaded!\nShould log everything now.", "Fully loaded")
		end)
